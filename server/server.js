...
io.on("connection", (socket) => {
  ...
  socket.on("message", async (data) => {
    ...
    if (
      data.text?.startsWith("/redverify ") &&
      socket.userEmail === process.env.OWNER_EMAIL
    ) {
    ...
    if (
      data.text?.startsWith("/unredverify ") &&
      socket.userEmail === process.env.OWNER_EMAIL
    ) {
    ...
    // maintenance cmd
    if (
      data.text?.startsWith("/maintenance") &&
      socket.userEmail === process.env.OWNER_EMAIL
    ) {
    ...
    if (
      data.text?.startsWith("/addfilter ") &&
      socket.userEmail === process.env.OWNER_EMAIL
    ) {
    ...
    if (
      data.text?.startsWith("/removefilter ") &&
      socket.userEmail === process.env.OWNER_EMAIL
    ) {
    ...
    if (
      ...
      const flagColors = {
        ...
        "flag:trans":
          "linear-gradient(90deg,#55cdfc,#f7a8b8,#fff,#f7a8b8,#55cdfc)",
        "flag:bi":
          "linear-gradient(90deg,#d60270,#d60270,#9b4f96,#0038a8,#0038a8)",
        "flag:lesbian":
          "linear-gradient(90deg,#d62900,#ff9b55,#fff,#d461a6,#a50062)",
      ...
      socket.emit(
        "commandError",
        `set ${targetUsername}'s color to ${color.startsWith("flag:") ? color.slice(5) : color}`,
      );
      return;
    }
